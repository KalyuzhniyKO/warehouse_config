...
- інвентаризацію з підрахунком фактичних залишків, завершенням через `adjustment`-рухи та експортом CSV/XLSX;
...
Повний цикл інвентаризації виконується у розділі **Інвентаризація** (`/stock/inventory/`):

1. **Створити інвентаризацію.** Адміністратор складу або комірник натискає **Нова інвентаризація**, вибирає склад і, за потреби, локацію. Система створює документ `InventoryCount` і рядки `InventoryCountLine` з поточними обліковими залишками.
2. **Внести фактичні залишки.** На сторінці **Підрахунок** відповідальний заповнює фактичну кількість і коментарі по рядках. На цьому етапі `StockBalance` не змінюється.
3. **Перевірити різницю.** Detail-сторінка показує облікову кількість, фактичну кількість, різницю, загальну кількість рядків, кількість розбіжностей, суму надлишків і суму нестач.
4. **Завершити інвентаризацію.** Після перевірки адміністратор складу або комірник натискає **Завершити інвентаризацію**.
5. **Автоматичне коригування.** Система створює рухи `StockMovement` типу `adjustment` тільки для рядків з різницею та прив'язує їх до інвентаризації.
6. **Експорт.** На сторінці інвентаризації доступні **Експорт CSV** і **Експорт XLSX**. CSV зберігається у UTF-8 з BOM для Excel, XLSX форматується через `openpyxl`, якщо бібліотека доступна. Експорт дозволений суперкористувачу, адміністратору складу, комірнику та ролі **Перегляд / аудитор**. Аудитор може експортувати, але не може створювати, редагувати або завершувати інвентаризацію.
...
4. **Переміщення товару** доступне у меню **Складські операції → Переміщення товару** або за URL `/stock/transfer/`. Форма викликає `transfer_stock`, створює рух типу `transfer`, зменшує залишок у локації-відправнику та збільшує залишок у локації-отримувачі.
5. **Рухи товарів** доступні за URL `/stock/movements/` із фільтрами за типом, номенклатурою, складом, локацією, датами та пошуком за назвою, `internal_code`, `barcode`.
6. **PDF етикетки** можна завантажити зі сторінки друку номенклатури `/labels/item/<id>/print/` або напряму `/labels/item/<id>/download/`. Стандартний шаблон — 58×40 мм.
7. **Друк через CUPS** використовує системну команду `lp -d PRINTER_NAME file.pdf`. Помилки друку записуються у `PrintJob` і показуються користувачу без 500-помилки.
...
- **Перегляд / аудитор** — перегляд залишків, рухів та експорт інвентаризацій без права створення, редагування або завершення.
...
Операція **Видача товару** доступна за маршрутом `/stock/issue/` для ролей **Адміністратор складу** та **Комірник**. Вона списує товар із вибраної локації через сервісну логіку складу, створює рух `out` і не дозволяє сформувати відʼємний залишок.

Під час видачі потрібно вибрати номенклатуру, склад, локацію, кількість і тип видачі: продаж, ремонтні роботи, виробничі потреби або інше. Додатково можна вказати цех / підрозділ, отримувача / відповідального, номер документа та коментар. Після успішної операції система показує залишок після видачі та переходи до нової видачі або журналу рухів.
