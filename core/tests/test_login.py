...
class TabletLoginScreenTests(TestCase):
    ...
    def test_ukrainian_login_page_is_tablet_friendly(self):
        ...
        self.assertContains(response, "login-brand-bar")
        self.assertContains(response, "login-wordmark__t")
        self.assertContains(response, 'name="username"')
        self.assertContains(response, 'class="form-control')
        self.assertContains(response, 'autocomplete="username"')
        self.assertContains(response, 'name="password"')
        self.assertContains(response, 'autocomplete="current-password"')
        self.assertContains(response, 'class="btn btn-primary login-submit w-100"')
