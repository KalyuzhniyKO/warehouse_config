...
def filter_balances(filters):
    queryset = StockBalance.objects.select_related(
        "item", "item__category", "location", "location__warehouse"
    )
    if filters.get("warehouse"):
...
def get_movement_summary(filters):
    aggregate = filter_movements(filters).aggregate(
        ...
        total_returns=Sum(
            "qty", filter=Q(movement_type=StockMovement.MovementType.RETURN)
        ),
        total_transfers=Sum("qty", filter=Q(movement_type__in=TRANSFER_TYPES)),
        unique_items=Count("item", distinct=True),
    )
    return {
        key: decimal_zero(value) if key.startswith("total_") else value
...
def get_movements_by_day(filters):
    return list(
        ...
        .annotate(
            ...
            writeoff_qty=Sum(
                "qty", filter=Q(movement_type=StockMovement.MovementType.WRITEOFF)
            ),
        )
        .order_by("day")
    )
