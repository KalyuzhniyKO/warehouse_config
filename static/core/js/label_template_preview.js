(function () {
  ...
  const refresh = () => {
    ...
    const pxPerMm = Math.max(2.4, Math.min(4.4, Math.min(stageWidth / width, stageHeight / height)));

    sheet.style.width = `${width * pxPerMm}px`;
    sheet.style.height = `${height * pxPerMm}px`;

    content.style.top = `${mt * pxPerMm}px`;
    content.style.right = `${mr * pxPerMm}px`;
    content.style.bottom = `${mb * pxPerMm}px`;
    content.style.left = `${ml * pxPerMm}px`;
    ...
    itemName.style.fontSize = `${Math.max(getValue('item_name_font_size', 8), 6)}pt`;
    internalCode.style.fontSize = `${Math.max(getValue('internal_code_font_size', 7), 6)}pt`;
    barcodeText.style.fontSize = `${Math.max(getValue('barcode_text_font_size', 7), 6)}pt`;

    barcode.style.display = '';
    barcode.style.height = `${barcodeHeight * pxPerMm}px`;
    barcode.style.setProperty('--bar-width', `${Math.max(barWidth * pxPerMm, 1)}px`);
    ...
    const textEstimateMm = (isChecked('show_item_name') ? getValue('item_name_font_size', 8) * 0.45 : 0)
      + (isChecked('show_internal_code') ? getValue('internal_code_font_size', 7) * 0.38 : 0)
      + (isChecked('show_barcode_text') ? getValue('barcode_text_font_size', 7) * 0.38 : 0)
      + 1.8;
    ...
    warningBarcode.hidden = !(barcodeHeight > innerHeight * 0.85 || innerWidth < 18);
    warningOverflow.hidden = !(contentEstimateMm > innerHeight);
    warningText.hidden = !(isChecked('show_item_name') && (innerWidth < 16 || getValue('item_name_font_size', 8) > innerHeight * 0.34));
  };
